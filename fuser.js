...
(async ()=>{
	...
	var modes = {
		devenv: async ()=> {
			...
			var res = await libx.bundler.exec([
				...
				`node ./node_modules/bundularjs/modules/fuser.js --build --serve --watch --clearLibs --secret=${secret} ${libx.node.args.api ? ' --api-run' : ''} ${libx.node.args.env ? ' --env='+libx.node.args.env:'' } ${libx.node.args.bare ? ' --bare' : '' }`, 
			], true);
	...
	if (libx.node.args.apiDeploy) {
		...
		modes.build();
	} else if (libx.node.args.dev || libx.node.args.bare) { 
		modes.devenv();
